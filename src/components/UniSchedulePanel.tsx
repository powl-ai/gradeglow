...
const colorOptions = [
  {
    value: "violet",
    label: "Violett",
    className: "bg-violet-100 text-violet-950 ring-violet-300",
  },
  {
    value: "pink",
    label: "Pink",
    className: "bg-fuchsia-100 text-fuchsia-950 ring-fuchsia-300",
  },
  {
    value: "blue",
    label: "Blau",
    className: "bg-sky-100 text-sky-950 ring-sky-300",
  },
  {
    value: "emerald",
    label: "Grün",
...
const formatTimeRange = (item: UniScheduleItem) => {
  const start =
    normalizeScheduleTime(item.startTime) || item.startTime || "offen";
...
export default function UniSchedulePanel({
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage(
      editingId
        ? "Stundenplan-Eintrag aktualisiert."
        : "Stundenplan-Eintrag gespeichert.",
    );
    ...
    <section className="space-y-5 sm:space-y-6">
      <div className="rounded-3xl bg-white/90 p-5 shadow-sm ring-1 ring-violet-100 backdrop-blur sm:p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-3xl bg-slate-950 p-4 text-white ring-1 ring-slate-900">
            ...
            <p className="mt-2 text-sm font-black text-slate-100">
              {isLoaded ? syncMessage : "Stundenplan lädt…"}
            </p>
          </div>
        </div>
        ...
        {(message || isTimeInvalid) && (
          <p
            className={`mt-4 rounded-2xl p-3 text-sm font-bold ring-1 ${isTimeInvalid ? "bg-rose-50 text-rose-700 ring-rose-100" : "bg-emerald-50 text-emerald-700 ring-emerald-100"}`}
          >
            {isTimeInvalid
              ? "Bitte Uhrzeiten im Format 08:00 oder 0800 eintragen."
              : message}
          </p>
        )}
      </div>
      ...
      {isFormOpen && (
        ...
        >
          <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm font-bold text-violet-700">
                {editingId ? "Eintrag bearbeiten" : "Neuer Uni-Termin"}
              </p>
            ...
            {editingId && (
              <button
                type="button"
                className="rounded-2xl bg-slate-50 px-4 py-2.5 text-sm font-black text-slate-600 ring-1 ring-slate-200"
                onClick={resetForm}
              >
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <label className="block xl:col-span-2">
              ...
              <input
                ...
                placeholder="z. B. Mikroökonomik Vorlesung"
                value={form.title}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
              ...
              <input
                ...
                onBlur={() =>
                  setForm((current) => ({
                    ...current,
                    startTime:
                      normalizeScheduleTime(current.startTime) ||
                      current.startTime,
                  }))
                }
              />
              ...
              <input
                ...
                onBlur={() =>
                  setForm((current) => ({
                    ...current,
                    endTime:
                      normalizeScheduleTime(current.endTime) || current.endTime,
                  }))
                }
              />
            </label>
            <label className="block">
              ...
              <input
                className="field-input"
                placeholder="Raum, Campus, online…"
                value={form.location}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
            ...
            <label className="block md:col-span-2 xl:col-span-4">
              ...
              <textarea
                ...
                placeholder="Dozent, Link, Vorbereitung, Besonderheiten…"
                value={form.notes}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
          ...
          <button
            type="submit"
            className="mt-5 rounded-2xl bg-violet-700 px-5 py-3 text-sm font-black text-white shadow-lg shadow-violet-100 transition hover:-translate-y-0.5 disabled:opacity-50"
            disabled={!form.title.trim() || isTimeInvalid}
          >
        ...
        <div className={`grid gap-3 ${scheduleWeekDays === 5 ? "xl:grid-cols-5" : scheduleWeekDays === 6 ? "xl:grid-cols-6" : "xl:grid-cols-7"}`}>
          {visibleWeekdayOptions.map((day) => {
            ...
            return (
              <div
                key={day.value}
                className="min-h-48 rounded-3xl bg-slate-50 p-3 ring-1 ring-slate-200"
              >
                  ...
                  {dayItems.length === 0 ? (
                    ...
                    dayItems.map((item) => {
                      ...
                      return (
                        <article
                          key={item.id}
                          className={`rounded-2xl p-2 ring-1 ${getColorClassName(item.color)} ${item.isHidden ? "opacity-60" : ""}`}
                        >
                          <div className="flex items-start gap-2">
                            <button
                              type="button"
                              className="min-w-0 flex-1 rounded-xl p-1 text-left transition hover:bg-white/40"
                            ...
                            >
                              ...
                              <p className="mt-2 text-[0.65rem] font-black uppercase tracking-[0.16em] opacity-55">
                                Tippen zum Bearbeiten
                              </p>
                            </button>
                            <button
                              type="button"
                              className="shrink-0 rounded-xl bg-white/70 px-2.5 py-1.5 text-xs font-black ring-1 ring-white/70"
                              onClick={(event) => {
                              ...
                              aria-label={`${item.title} Aktionen`}
                            >
                              ⋯
                            </button>
                          </div>
                          {actionsOpen && (
                            <div className="mt-2 grid gap-1.5">
                              <button
                                type="button"
                                className="rounded-xl bg-white/80 px-2.5 py-1.5 text-left text-[0.68rem] font-black ring-1 ring-white/80"
                                onClick={() => toggleHidden(item.id)}
                              >
                                {item.isHidden ? "Einblenden" : "Ausblenden"}
                              </button>
                              <button
                                type="button"
                                className="rounded-xl bg-white/80 px-2.5 py-1.5 text-left text-[0.68rem] font-black text-rose-700 ring-1 ring-white/80"
